  ...
  <div style={{ display: 'flex', gap: 12, padding: '14px 0', borderBottom: '1px solid #f1f5f9', alignItems: 'flex-start' }}>
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        {tag && <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: tagColor || '#ede9fe', color: '#1A56DB' }}>{tag}</span>}
      </div>
...
const Dashboard = () => {
  ...
  // ── Client stats ─────────────────────────────────────────────
  const clientStats = [
    { icon: '📌', label: 'Active Gigs',         value: stats?.activeGigs        ?? 4,  trend: 0,   color: '#1E40AF' },
  ...
  const freelancerStats = [
    { icon: '📋', label: 'Active Proposals',    value: stats?.activeProposals   ?? 7,  trend: 12,  color: '#1E40AF' },
  ...
  // ── Quick actions ─────────────────────────────────────────────
  const clientActions = [
    { icon: '➕', label: 'Post a Job', to: '/create-gig', color: '#1E40AF', bg: '#ede9fe' },
  ...
  const freelancerActions = [
    { icon: '🔍', label: 'Browse Jobs',  to: '/gigs',         color: '#1E40AF', bg: '#ede9fe' },
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ width: 40, height: 40, border: '3px solid #e0e7ff', borderTopColor: '#1E40AF', borderRadius: '50%', animation: 'spin 0.7s linear infinite', margin: '0 auto 12px' }} />
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: '-40%', right: '-5%', width: 400, height: 400, background: 'radial-gradient(circle, rgba(26,86,219,0.2) 0%, transparent 65%)', pointerEvents: 'none' }} />

        <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
            <div>
              <p style={{ color: '#93C5FD', fontSize: 13, marginBottom: 6, fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {isFreelancer ? (
                <Link to="/gigs"><button style={{ padding: '10px 22px', borderRadius: 10, background: 'linear-gradient(135deg,#1A56DB,#22D3EE)', border: 'none', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 14px rgba(26,86,219,0.4)' }}>🔍 Browse Jobs</button></Link>
              ) : (
                <Link to="/create-gig"><button style={{ padding: '10px 22px', borderRadius: 10, background: 'linear-gradient(135deg,#1A56DB,#22D3EE)', border: 'none', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 14px rgba(26,86,219,0.4)' }}>+ Post a Job</button></Link>
              )}
      ...
      <div style={{ maxWidth: 1200, margin: '24px auto 60px', padding: '0 48px', display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '22px 24px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              ...
              <Link to={isFreelancer ? '/my-proposals' : '/my-gigs'} style={{ fontSize: 13, color: '#1E40AF', fontWeight: 600, textDecoration: 'none' }}>View all →</Link>
            </div>
            ...
            <div style={{ marginTop: 12, textAlign: 'center', padding: '24px 0', display: 'none' }}>
              ...
              <Link to={isFreelancer ? '/gigs' : '/create-gig'}>
                <button style={{ marginTop: 10, padding: '8px 20px', borderRadius: 10, background: '#1E40AF', border: 'none', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            {/* Banner */}
            <div style={{ height: 70, background: 'linear-gradient(135deg, #1E40AF, #1A56DB)', position: 'relative' }}>
            ...
            <div style={{ padding: '0 20px 20px' }}>
              <div style={{ marginTop: -28, marginBottom: 12 }}>
                <div style={{ width: 56, height: 56, borderRadius: 14, background: 'linear-gradient(135deg,#1A56DB,#22D3EE)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 900, color: '#fff', border: '3px solid #fff', boxShadow: '0 4px 12px rgba(26,86,219,0.3)' }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 999, background: '#ede9fe', color: '#1A56DB', textTransform: 'capitalize' }}>{user?.role}</span>
            </div>
          </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: '#1E40AF' }}>75%</span>
            </div>
            <div style={{ height: 8, background: '#f1f5f9', borderRadius: 4, marginBottom: 16, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: '75%', background: 'linear-gradient(90deg, #1E40AF, #22D3EE)', borderRadius: 4, transition: 'width 0.8s ease' }} />
            </div>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <div style={{ width: 20, height: 20, borderRadius: '50%', background: item.done ? '#1E40AF' : '#f1f5f9', border: item.done ? 'none' : '1.5px solid #d1d5db', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <Link to="/settings">
              <button style={{ width: '100%', padding: '10px', borderRadius: 10, background: '#f5f3ff', border: '1px solid #ede9fe', color: '#1E40AF', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', marginTop: 6, transition: 'all 0.2s' }}
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            ...
            {[
              ...
              { title: 'API Integration', date: 'Apr 10', daysLeft: 17, color: '#1E40AF', bg: '#ede9fe' },
            ].map((item, i) => (
          ...
          {/* CTA Banner */}
          <div style={{ background: 'linear-gradient(135deg, #1E40AF, #1A56DB)', borderRadius: 16, padding: '20px', textAlign: 'center', boxShadow: '0 4px 14px rgba(26,86,219,0.3)' }}>
